...
fun deleteMarked(inputName: String, outputName: String) {
    val writer = File(outputName).bufferedWriter()
    for (line in File(inputName).readLines()) {
        if (line.isEmpty() || line.get(0).isLetter()) {
            writer.write(line)
            writer.newLine()
        }
    }
...
//    val words = listOf('ж','ч','ш','щ')
//    val trueWords = mapOf('ы' to 'и','я' to 'а', 'ю' to 'у')
//val writer = File(outputName).bufferedWriter()
//    for ( line in File(inputName).readLines()) {
//    }
...
fun centerFile(inputName: String, outputName: String) {
    ...
    for (line in File(inputName).readLines()){
        ...
        sb.append(line.trim())
        sb.append("\n")
        max = maxOf(l.length,max)
    }
    val writer = File(outputName).bufferedWriter()
    for (line in sb.toString().trim().lines()) {
        val offset = " ".repeat((max - line.length)/2)
        writer.write(offset + line)
        writer.newLine()
    }
    writer.close()
}
